...
export class CoverageReporterService {
  ...
  createSummary(coverage, baselineCoverage, minimumCoverage) {
    ...
    if (coverage.type === 'packages') {
      return this.createPackageComparisonSummary(coverage, baselineCoverage, minimumCoverage);
    }
  ...
  createPackageComparisonSummary(currentCoverage, baselineCoverage, minimumCoverage) {
    ...
    for (const { package: pkgName, path: filePath } of currentCoverage.files) {
      try {
        const pkgCoverageData = JSON.parse(this.fs.readFileSync(filePath, 'utf8'));
        ...
        packageComparisons.push({
          ...
          status: this.calculateOverallCoverage(pkgCoverage) >= minimumCoverage ? 'pass' : 'fail'
        });
        ...
        console.warn(`⚠️ Failed to parse current coverage for ${pkgName}: ${error.message}`);
      }
    }

    // Calculate overall averages
    const overallCoverage = totalPackages > 0 ? {
  ...
  logFinalStats(summary, baselineCoverage) {
    ...
    if (summary.type === 'packages') {
      ...
      for (const pkg of summary.packages) {
        ...
        const baselineOverall = pkg.baseline ? this.calculateOverallCoverage(pkg.baseline) : null;
        
        if (baselineOverall !== null) {
          const diff = overallCoverage - baselineOverall;
          const diffIcon = diff > 0 ? '⬆️' : diff < 0 ? '⬇️' : '➡️';
          console.log(`  📋 ${pkg.package}: ${overallCoverage.toFixed(2)}% (${diff > 0 ? '+' : ''}${diff.toFixed(2)}% ${diffIcon})`);
        } else {
  ...
  generateMarkdownReport(coverage, minimumCoverage, baselineCoverage = null) {
    ...
    if (coverage.type === 'packages') {
      return this.generatePackageMarkdownReport(coverage, minimumCoverage, baselineCoverage);
    }
  ...
  generatePackageMarkdownReport(summary, minimumCoverage, baselineCoverage = null) {
    const getStatus = (percentage) => {
      if (percentage >= 80) return '✅ Good';
      if (percentage >= 60) return '⚠️ Fair';
      return '❌ Poor';
    ...
    for (const pkg of summary.packages) {
      ...
      const overallCoverage = this.calculateOverallCoverage(coverage);
      const baselineOverall = baseline ? this.calculateOverallCoverage(baseline) : null;
    ...
    const failedPackages = summary.packages.filter(pkg => pkg.status === 'fail');
  ...
  getCoverage(inputs) {
    ...
    // Try to load from coverage-artifacts directory first (generated by previous step)
    const currentCoverage = this.loadCurrentCoverage(inputs.outputDir);
    if (currentCoverage) {
    ...
    if (!inputs.coverageCommand) {
      throw new Error('Coverage command not provided and no coverage artifacts found');
    }
  ...
  loadCurrentCoverage(outputDir) {
    try {
        ...
        for (const item of dirContents) {
          ...
          if (this.fs.statSync(itemPath).isDirectory()) {
            ...
            if (this.fs.existsSync(coverageJsonPath)) {
              packagesWithCoverage.push({ package: item, path: coverageJsonPath });
            }
          }
        }
      }
      ...
      if (packagesWithCoverage.length > 0) {
        console.log(`📦 Found ${packagesWithCoverage.length} current coverage packages`);
        return { type: 'packages', files: packagesWithCoverage };
      }

      // Fallback: try to read from standard coverage-summary.json
      const summaryPath = path.join(outputDir, 'coverage-summary.json');
      if (this.fs.existsSync(summaryPath)) {
        console.log('📊 Using coverage-summary.json');
