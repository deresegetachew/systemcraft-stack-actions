...
export class CoverageReporterService {
  ...
  normalizeInputs(inputs = {}) {
    return {
      ...
      baselineArtifactName: inputs.baselineArtifactName || 'coverage-baseline-main',
  ...
  getCoverage(inputs) {
    // First priority: Try to load from coverage-artifacts directory (generated by previous step)
    const currentCoverage = this.loadCurrentCoverage(inputs.outputDir, inputs.coverageFile);
    if (currentCoverage) {
      console.log('✅ Using coverage data from artifacts directory');
      return currentCoverage;
    }

    // Second priority: Load from specified coverage file
    ...
    // Last resort: Run coverage command to generate fresh coverage
    ...
    if (this.shouldLoadCoverageFromFile(inputs.coverageFile)) {
      console.log(`📄 Loading coverage from generated file: ${inputs.coverageFile}`);
      return this.readCoverageFromFile(inputs.coverageFile);
    }

    // Also try to load from artifacts directory in case the command generated artifacts
    const generatedCoverage = this.loadCurrentCoverage(inputs.outputDir, inputs.coverageFile);
    if (generatedCoverage) {
      console.log('✅ Using coverage data generated in artifacts directory');
      return generatedCoverage;
    }

    // Fallback: parse the command output (less reliable)
  ...
  loadCurrentCoverage(outputDir, coverageFile) {
    try {
      ...
      if (this.fs.existsSync(summaryPath)) {
        console.log(`📊 Using ${path.basename(coverageFile)} from output directory`);
