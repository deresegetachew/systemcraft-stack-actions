  ...
  describe('main()', () => {
    ...
    describe('multi-release mode', () => {
      ...
      it('should create a new release branch based on the plan file', async () => {
        ...
        // -- Assert
        assert.strictEqual(
          mockGitService.checkRemoteBranch.mock.callCount(),
          1,
        );
      ...
      it('should skip branch creation if branch already exists', async () => {
        ...
        assert.strictEqual(
          mockGitService.createBranch.mock.callCount(),
          0,
          'Should not create branch',
        );
        assert.strictEqual(
          mockGitService.pushBranch.mock.callCount(),
          0,
          'Should not push branch',
        );
      });
...
describe('configureAuthEnv', () => {
  ...
  it('applies provided npm and github tokens to env and process', () => {
    ...
    assert.strictEqual(result.NODE_AUTH_TOKEN, 'node-token');
    assert.strictEqual(process.env.NODE_AUTH_TOKEN, 'node-token');
