...
describe('ShellUtil', () => {
  ...
  describe('exec()', () => {
    it('should return an object with stdout when command succeeds', () => {
      ...
      assert.strictEqual(
        mockCPApi.execSync.mock.calls[0].arguments[0],
        'test command',
      );
    });
    ...
    it('should return an object with stdout when stdio is pipe (Buffer output)', () => {
      mockCPApi.execSync.mock.mockImplementationOnce(() =>
        Buffer.from('piped output'),
      );

      const result = shellUtil.exec('test command with pipe', {
        stdio: 'pipe',
      });

      assert.deepStrictEqual(result, { stdout: 'piped output' });
      assert.strictEqual(mockCPApi.execSync.mock.callCount(), 1);
      assert.deepStrictEqual(mockCPApi.execSync.mock.calls[0].arguments[1], {
        stdio: 'pipe',
      });
    });
