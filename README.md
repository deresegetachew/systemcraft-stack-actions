...
- `enable-multi-release` (optional): Controls multi-release mode (`'false'` by default).
- `node-auth-token` (required): NPM authentication token for publishing.
  ...
  publish:
    ...
    steps:
      ...
      - uses: deresegetachew/systemcraft-stack-actions/actions/release-with-branching@main
        with:
          enable-multi-release: ${{ vars.ENABLE_MULTI_RELEASE }}
